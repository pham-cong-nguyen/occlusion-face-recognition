...
from .layers import (
    ...
    CosMarginPenaltyLogists
)
from modules.resnet import build_resnet
    ...
    def backbone(x_in):
        if backbone_type == 'ResNet50':
            ...
            return MobileNetV2(input_shape=x_in.shape[1:], include_top=False,
                               weights=weights)(x_in)
        elif backbone_type == 'MobileNetV2_075':
            return MobileNetV2(input_shape=x_in.shape[1:], include_top=False,
                               weights=weights, alpha=0.75)(x_in)
        # elif backbone_type == 'EfficientNetB4':
        #     return EfficientNetB4(input_shape=(112,112,3), include_top=False)(x_in)
        elif backbone_type == 'ResNet34':
            return build_resnet(include_top=False, input_shape=x_in.shape[1:])(x_in)
        else:
...
def CosHead(num_classes, margin=0.35, logist_scale=64, name='ArcHead'):
    """Arc Head"""
    def arc_head(x_in, y_in):
        x = inputs1 = Input(x_in.shape[1:])
        y = Input(y_in.shape[1:])
        x = CosMarginPenaltyLogists(num_classes=num_classes,
                                    margin=margin,
                                    logist_scale=logist_scale)(x, y)
        return Model((inputs1, y), x, name=name)((x_in, y_in))
    return arc_head
